...
namespace Day2
{
    class Program
    {
        const string input = @"8-9 x: xxxxxxxrk
5-8 f: fxffmfff
2-5 c: ccspwpc
1-4 g: gggg
9-10 l: lllllllllk
...
";
        ...
        {
            public Policy(int min, int max, char character)
            {
                Min = min;
                Max = max;
                Character = character;
            ...
            public bool Match(string password)
            {
                ...
                int nb = password.Count(c => c == expected);
                return nb >= Min && nb <= Max;
            }
            public bool Match2(string password)
            {
                char expected = Character;
                //int nb = password.Count(c => c == expected);
                return ( Min <= password.Length && password[Min-1] == expected) ^ ( Max <= password.Length && password[Max-1] == expected);
            }
        ...
        public static Policy ParsePolicy(string s)
        {
            ...
            string[] minMax = a[0].Split('-', StringSplitOptions.RemoveEmptyEntries);
            return new Policy(Int32.Parse(minMax[0]), Int32.Parse(minMax[1]), a[1][0]);
        }

        private const string test = @"1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc";
        static void exo1()
        {
            ...
            var lins = lines.Select(s => s.Split(':', StringSplitOptions.RemoveEmptyEntries)).ToArray();
            int nbMatch = lins
                .Count(a => ParsePolicy(a[0].Trim()).Match2(a[1].Trim()));
            Console.Out.WriteLine(nbMatch);
