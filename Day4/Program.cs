...
namespace Day4
{
    class Program
    {
        private const string input = @"hgt:159cm
...
pid:921038878
";
        private const string test = @"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
iyr:2011 ecl:brn hgt:59in";
        private const string test2Valid = @"pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719";
        private const string test2Invalid = @"eyr:1972 cid:100
...
pid:3556412378 byr:2007";
        ...
        private static string[] OptionalFields = new []{"cid"};
        ...
        static void exo1()
        {
            ...
            foreach (var line in lines) {
                if (line.Length == 0) {
                    if (RequiredFields.All(f => entries.ContainsKey(f))) {
                        res++;
                    }
        ...
        static  bool ValidPassport( IReadOnlyDictionary<string, string> dictionary)
        {
            int nbValidFields = 0;
            if (RequiredFields.All(dictionary.ContainsKey))
            {
                foreach (var field in RequiredFields)
                {
                    var val = dictionary[field];
                    bool valid = field switch
                    {
                        "byr" => ValidateYear(val, 1920, 2002),
                        "iyr" => ValidateYear(val, 2010, 2020),
                        "eyr" => ValidateYear(val, 2020, 2030),
                        "hgt" => ValidateHeight(val),
                        "hcl" => ValidateHairColor(val),
                        "ecl" => ValidateEyeColor(val),
                        "pid" => ValidatePasswordNumber(val),
            ...
            return nbValidFields == RequiredFields.Length;
        }
        static bool ValidateYear(string val, int min, int max) {
            Regex year = new Regex("^[0-9]{4}$");
            if (!year.IsMatch(val)) {
                return false;
            }
            int y = Int32.Parse(val);
            return y >= min && y <= max;
        }
        static bool ValidateHeight(string val) {
            Regex year = new Regex("^(?<NumberVal>[0-9]+)(?<TypeSystem>(cm|in))$");
            var match = year.Match(val);
            if (!match.Success) {
                return false;
            }

            var type = match.Groups["TypeSystem"].Value;
            switch (type)
            {
                case "cm":
                    int cm = Int32.Parse(match.Groups["NumberVal"].Value);
                    return cm >= 150 && cm <= 193;
                case "in":
                    int inch = Int32.Parse(match.Groups["NumberVal"].Value);
                    return inch >= 59 && inch <= 76;
            }

            throw new Exception();
        }
        static bool ValidateHairColor(string val) {
            Regex year = new Regex("^#[0-9a-f]{6}$");
            if (!year.IsMatch(val)) {
                return false;
            }
        ...
        static bool ValidatePasswordNumber(string val) {
            Regex year = new Regex("^[0-9]{9}$");
            if (!year.IsMatch(val)) {
                return false;
            }
        ...
        static bool ValidateEyeColor(string val) {
            return ValidEyeColors.Any(v => v == val);
        }
    }
}