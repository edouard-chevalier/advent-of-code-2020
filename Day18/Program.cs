...
((2 + 3 * 6 * 2) * 5 * 7 * 7 * 2) + 4 + 2 * 6 + 5 + (6 + 7 * 4 + 5)";
        ...
        private int[] expected = new int[] {51, 9, 26, 437, 12240, 13632,176498,11200 };
        ...
        static void exo1()
        {
            ...
            char CurrentChar(string expr, ref int pointer)
            {
                while (pointer < expr.Length && expr[pointer] == ' ')
                {
                    pointer++;
                }
            ...
            long Eval(string expr, ref int pointer)
            {
                var currentChar = CurrentChar( expr, ref pointer);
                long currentRes ;
                if (currentChar == '(') {
                    
                    currentRes = Eval(expr, ref pointer);
                }
                else
                {
                    currentRes = currentChar - '0';
                }

                currentChar = CurrentChar( expr, ref pointer);
                
                while (currentChar != ')' && pointer < expr.Length)
                {
                    
                    char op = currentChar;
                    var rightOpChar = CurrentChar( expr, ref pointer);
                    long rightVal;
                    if (rightOpChar == '(') {
                    
                        rightVal = Eval(expr, ref pointer);
                    }
                    else
                    {
                        rightVal = rightOpChar - '0';
                    }
         ...
         static void exo2()
        {
            ...
            long Eval(string expr, ref int pointer)
            {
                ...
                while (currentChar != ')' && pointer < expr.Length)
                {
                    ...
                    char op = currentChar;
                    operators.Add(op);
                    var operandChar = CurrentChar( expr, ref pointer);
                    if (operandChar == '(') {
                        currentRes = Eval(expr, ref pointer);
                    }
                    else
                    {
                        currentRes = operandChar - '0';
                    }
                    operands.Add(currentRes);
                ...
                {
                    throw new Exception("opes");
                }

                int nbOperations = operators.Count;
                for (int i = 0; i < nbOperations; i++)
                {
                    if (operators.Contains('+'))
                    {
                        int idx = operators.IndexOf('+');
                        var newOperands = new List<long>();
                        newOperands.AddRange(operands.Take(idx));
                        newOperands.Add(operands[idx] + operands[idx + 1]);
                        newOperands.AddRange(operands.Skip(idx + 2));
