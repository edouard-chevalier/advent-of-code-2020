...
";
        const string test = @"mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
...
sqjhc mxmxvkd sbzzf (contains fish)";
        ...
        private static void exo1()
        {
            ...
            var regex = new Regex("^(?<Ingredients>[a-z ]+)\\(contains (?<Allergens>[a-z ,]+)\\)$");
            var foods = new List<(string[] Ingredients, string[] Allergens)>();
            foreach (var line in lines) {
                ...
                foods.Add((match.Groups["Ingredients"].Value.Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(s=>s.Trim()).ToArray(),
                    match.Groups["Allergens"].Value.Split(",", StringSplitOptions.RemoveEmptyEntries).Select(s=>s.Trim()).ToArray()));

            }

            var allergenToPossibleIngredients = new Dictionary<string, HashSet<string>>();
            var ingredientFrequency = new Dictionary<string, int>();

            foreach (var (ingredients, allergens) in foods) {
                foreach (var allergen in allergens) {
                    if (allergenToPossibleIngredients.TryGetValue(allergen, out var pi)) {
                        pi.IntersectWith(ingredients);
                    }
                    else
                    {
                        allergenToPossibleIngredients[allergen] = new HashSet<string>(ingredients);
                    }
                }

                foreach (var ingredient in ingredients)
                {
                    if (!ingredientFrequency.TryGetValue(ingredient, out var f))
                    {
                        f = 0;
                    }

                    ingredientFrequency[ingredient] = f + 1;
                }
            }

            int sum = 0;
            foreach (var kv in ingredientFrequency)
            {
                if (allergenToPossibleIngredients.Values.Any(v => v.Contains(kv.Key)))
                {
                    continue;
                }
        ...
        private static void exo2()
        {
            ...
            {
                var foundAllergen = allergenToPossibleIngredients.Where(kv => kv.Value.Count == 1).Select(kv => kv.Key)
                    .ToArray();
                if (foundAllergen.Length == 0) {
                    throw new Exception("converge no");
                }

                foreach (var a in foundAllergen)
                {
                    var ingredient = allergenToPossibleIngredients[a].First();
                    ingredientToAllergen[ingredient] = a;

                    allergenToPossibleIngredients.Remove(a);
                    foreach (var kv in allergenToPossibleIngredients)
                    {
                        kv.Value.Remove(ingredient);
                    }
                }
            }
            ...
            var res = new List<(string Ingredient, string Allergen)>(
                ingredientToAllergen.Select(kv => (kv.Key, kv.Value)));

            res.Sort( ((tuple, valueTuple) => tuple.Allergen.CompareTo(valueTuple.Allergen)));
            foreach (var kv in res)
            {
                Console.WriteLine($"{kv.Ingredient}:{kv.Allergen}");
            }
            Console.WriteLine(string.Join(",",res.Select(v =>v.Ingredient)));
