...
";
        private const string test = @"light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.";
        ...
        private const string test2 = @"shiny gold bags contain 2 dark red bags.
...
dark violet bags contain no other bags.";
        ...
        static void exo1() {
            ...
            var reverseTree = new Dictionary<string, List<string>>();
            foreach (var line in lines)
            {
                var index = line.IndexOf("contain", StringComparison.Ordinal);
                var bagId = line.Substring(0, index - 5).Trim();
                Console.Out.WriteLine(bagId);

                var containedBags = line.Substring(index +"contain".Length).Trim();
                Console.Out.WriteLine(containedBags);
                if (containedBags.Contains("no other bags")) {
                    bags[bagId] = new Bag(bagId, Array.Empty<string>(),null);
                }
                else {
                    var containedBagsList = new List<string>();
                    var subBags = containedBags.Split(",", StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim())
                        .ToArray();

                    foreach (var subBag in subBags)
                    {
                        var elements = subBag.Split(" ", StringSplitOptions.RemoveEmptyEntries);
                        if (elements.Length != 4) {
                            throw new Exception(containedBags);
                        }
                        int nb = Int32.Parse(elements[0].Trim());
                        if (!elements[3].Trim().StartsWith("bag")) {
                            throw new Exception(containedBags);
                        }

                        var s = $"{elements[1].Trim()} {elements[2].Trim()}";
                        containedBagsList.Add(s);
                        if (!reverseTree.TryGetValue(s, out var parents)) {
                            parents = new List<string>();
                            reverseTree[s] = parents;
                        }
                        parents.Add(bagId);
                    }
                    bags[bagId] = new Bag(bagId, containedBagsList,null);
                }
            }
            var allParents = new HashSet<string>();

            void CollectParents(string s) {
                if (reverseTree.TryGetValue(s, out var parents)) {
            ...
            CollectParents("shiny gold");
            Console.Out.WriteLine(allParents.Count);
        ...
        static void exo2() {
            ...
            foreach (var line in lines)
            {
                ...
                Console.Out.WriteLine(containedBags);
                if (containedBags.Contains("no other bags")) {
                    bags[bagId] = new Bag(bagId, Array.Empty<string>(), Array.Empty<int>());
                }
                else {
                    var containedBagsList = new List<string>();
                    var containedBagsListN = new List<int>();
                    var subBags = containedBags.Split(",", StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim())
                        .ToArray();
                    ...
                    bags[bagId] = new Bag(bagId, containedBagsList, containedBagsListN);
                }
            }
            var cache = new Dictionary<string, int>();

            int CollectParents(string s) {
                if (cache.TryGetValue(s, out int res)) {
                    return res;
                }
                ...
                for (int i = 0; i <  bag.ContainedBags.Count; i++) {
                    sum += bag.NbBags[i] * ( CollectParents(bag.ContainedBags[i]) + 1);
                }
                cache[s] = sum;
                return sum;

            }
            int res = CollectParents("shiny gold");
        ...
        public class Bag
        {
            public Bag(string id, IReadOnlyList<string> containedBags, IReadOnlyList<int> nbBags)
            {
                ID = id;
            ...
            public IReadOnlyList<string> ContainedBags { get; }
            public IReadOnlyList<int> NbBags { get; }
        }
    }
}