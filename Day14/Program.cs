using System;
using System.Collections.Generic;
using System.Linq;
using System.Text.RegularExpressions;

namespace Day14
{
    class Program
    {
        private const string input = @"mask = X101011X011X10101011000001X00XX0X000
mem[60126] = 9674686
mem[39254] = 523988
mem[54849] = 40771927
mem[29690] = 10110
mem[10782] = 975308
mem[43128] = 4347
mask = 1X0X00101111100010100X001010XX0X0XXX
mem[60704] = 43881206
mem[63842] = 12369309
mem[45876] = 33941457
mem[7001] = 347
mem[57168] = 5484326
mem[9010] = 1526598
mask = XX0X0111X1101X1X101X110001100X101X00
mem[64667] = 9335
mem[557] = 103838228
mask = 01110111011110X0101X110000100X10100X
mem[37083] = 252803732
mem[38441] = 264510
mem[10471] = 6258763
mem[6599] = 7469003
mem[31917] = 4440673
mask = X011X01000X110X1X00X0100X0X000100011
mem[34590] = 1012
mem[45359] = 8432
mem[3178] = 33474
mem[3722] = 3556
mem[49428] = 9026812
mem[10938] = 35041698
mask = 01X1X11X011X1X0X11X0100X11101X010011
mem[42766] = 6617
mem[49563] = 960913
mem[47263] = 269
mem[31711] = 43288638
mem[41482] = 64610360
mem[16665] = 600
mem[32730] = 37650
mask = 00XX0X101111XXX1101X0X0X001000101100
mem[34707] = 40982522
mem[9182] = 545101093
mem[46509] = 44467432
mem[14087] = 1863
mem[40214] = 126903285
mem[60120] = 56379991
mask = 01XX10110110100010X100X0X10111100000
mem[30183] = 134656354
mem[49417] = 78985543
mem[14770] = 18142096
mem[61486] = 39482
mem[52941] = 98022933
mem[46433] = 952154
mask = 00010X10X1110010101001011001X100X101
mem[20851] = 938875
mem[42517] = 2746639
mem[18095] = 623480
mem[45359] = 554
mem[52625] = 9537360
mem[3178] = 634749644
mem[11612] = 1338
mask = 01000X1X111XX110101X10000X0XX11011X1
mem[33690] = 133806
mem[11082] = 47091974
mem[18025] = 716988
mem[1627] = 3080902
mask = 010101X1111XX11010X00000XX11XX0X0010
mem[1839] = 55761
mem[34016] = 19521
mem[32399] = 2550
mem[56670] = 486944499
mask = 0100101101110XX01011X100101111X10011
mem[57556] = 1980
mem[15099] = 1000
mem[2248] = 11713417
mask = 0X0X011X1111X110101X0000001000101X00
mem[28478] = 2335
mem[49509] = 262
mem[18296] = 16275
mem[3851] = 228035
mem[30877] = 53163521
mask = 000X01X010100X1010101101100000X0X100
mem[38075] = 79529
mem[42738] = 45415
mem[24109] = 1567958
mem[17143] = 93137
mem[45101] = 814797986
mask = XX01XX1X011X10X0101011000000010000X1
mem[1576] = 443528583
mem[36865] = 148
mem[46509] = 482
mem[62388] = 15
mem[63623] = 539909442
mem[41370] = 128
mem[44715] = 22308
mask = 11011X1000101X00XXX001X110X00100010X
mem[35152] = 34820483
mem[16033] = 65763
mem[1882] = 1027800
mask = 00010X10111110X0X010X0110010X01X1X01
mem[59815] = 2490477
mem[49157] = 6507122
mem[30800] = 590164239
mem[59044] = 165654579
mask = 01010X1111111X101010011X0011X0001011
mem[10137] = 228
mem[2311] = 13276802
mask = XX01X1X101101010101100001X000010000X
mem[34583] = 1655642
mem[11286] = 132679008
mem[22109] = 157717238
mask = 0001001011111X1X0X101XX11X1X101100X1
mem[57742] = 26691
mem[6874] = 10458032
mem[53037] = 2388387
mem[12331] = 5770
mem[51728] = 2135
mem[9580] = 487489805
mask = 01001X11011110X0X0X01X1100000111XXX1
mem[53769] = 55803305
mem[56229] = 24695063
mem[19171] = 48736390
mem[64782] = 735
mem[3081] = 280948
mem[57365] = 846847157
mask = 0X000110X1X11X1X10111000X010000XXX00
mem[61422] = 1126
mem[22207] = 67628317
mem[9539] = 61546790
mem[1131] = 130904490
mem[52684] = 8379637
mem[24366] = 383662099
mask = 010X1111011X1000X0X11X101X00101X1010
mem[18109] = 6735
mem[32797] = 531499843
mem[53585] = 432
mask = 100000X0111X100X1XX000X0101100110000
mem[7943] = 196928
mem[22176] = 12548
mem[30990] = 446363
mem[160] = 22025
mem[4615] = 566343
mask = 0X11011101X1X0011X011101111101001001
mem[39520] = 714461
mem[45101] = 3821808
mem[62323] = 6386948
mask = 01X0110X01XX100010110010011001000X01
mem[37344] = 447
mem[3023] = 855524337
mem[23128] = 112513310
mem[6564] = 11
mask = 0100X01XX111XX1X10111100001110001X10
mem[62178] = 63123
mem[55172] = 153103
mem[51929] = 181344
mem[34661] = 1972
mem[54999] = 1068
mem[59358] = 992
mask = 1001011XX0111XX010100X0001001X11X0X0
mem[41893] = 479
mem[6599] = 1981844
mem[46760] = 216449
mem[50221] = 811
mask = X0X10010111X101001100X01X01X11101X10
mem[25689] = 136
mem[10938] = 28382
mem[16643] = 197364
mem[51281] = 617
mem[16153] = 3676
mem[26490] = 384
mask = 010001X00X11100011100010111010X010X0
mem[64133] = 208
mem[24062] = 903539668
mem[36044] = 2001
mem[23144] = 1011029
mem[22716] = 11499
mem[59581] = 17899405
mem[26171] = 111934167
mask = 010010110X101000101XXX00000111001101
mem[43291] = 3112
mem[19446] = 5600478
mem[62398] = 341768545
mem[30567] = 465047
mem[38317] = 22125824
mask = 00X101101111101000X011X1X00X10X0X101
mem[53713] = 409555
mem[55388] = 65118
mem[54952] = 133834612
mem[25132] = 42713528
mem[16165] = 2107530
mask = 101101111111101100X0X111X1000X101X1X
mem[46469] = 6516
mem[58152] = 331216697
mem[1931] = 23454
mem[63623] = 304198968
mem[5559] = 32986355
mask = 0X0111XX01111X00101X10000X10010010X1
mem[54268] = 619
mem[50248] = 882
mem[12816] = 850402
mask = 010X0110011X100X1X10X1101X1XX0X11101
mem[37158] = 8631
mem[5913] = 1852245
mem[39724] = 809871
mem[27706] = 2649386
mem[53037] = 187717225
mask = 00011X110110101010X0X11100X11X1X1000
mem[22063] = 97071
mem[62330] = 777
mem[13645] = 8634786
mask = 0XXXX111011110X0101X100X010001X0X000
mem[3609] = 1585
mem[55533] = 3654
mem[46561] = 27985772
mem[49761] = 1723550
mem[9580] = 123963
mem[30529] = 15486580
mask = 01011111011X101X1011100X001101100X00
mem[386] = 5196729
mem[26441] = 930769
mem[51597] = 160686
mem[34426] = 89786878
mem[7046] = 1642552
mem[58359] = 19998
mask = X01110X1X11X1X0X100110XXX11101000011
mem[52684] = 413134467
mem[41057] = 237240996
mem[4615] = 1050604472
mem[37298] = 464
mem[10624] = 166667
mem[51214] = 189464
mask = 0011111101X01000101111XXX101X1101001
mem[57892] = 4128
mem[23904] = 1230477
mem[37354] = 295321
mem[9213] = 18002
mem[19975] = 93077192
mask = 1101011101X01111100010XX01X001000010
mem[9629] = 585
mem[14618] = 414
mem[46327] = 110854046
mem[24296] = 101
mem[15485] = 3005564
mask = X001XX1X0010X010101X1101100011000X01
mem[41309] = 445635
mem[48838] = 55661
mem[46592] = 765710
mem[18993] = 4196275
mask = 10010011X0100010X0100X01000X10001001
mem[56025] = 1496
mem[41771] = 3244
mem[19132] = 888
mem[65367] = 208629904
mem[59728] = 9087603
mask = 0101X1X0011010011X1001X111111XX11X01
mem[58407] = 87107553
mem[28815] = 68182
mem[28313] = 2239
mem[39552] = 101470
mask = 0111011101101X01X001001111010111XXXX
mem[8813] = 400372
mem[45602] = 32028274
mem[24857] = 114
mem[61754] = 95
mem[30298] = 580278
mask = 11011X110X1010X010X001010X0XX10X0011
mem[55] = 1059
mem[52684] = 56819312
mem[51597] = 2017
mem[39454] = 931156481
mem[62166] = 62175870
mem[36867] = 470
mem[47415] = 1746
mask = 11010010XX1X10X0101XX1X001000110000X
mem[1579] = 483360842
mem[28054] = 6824
mem[2134] = 7557
mem[47741] = 701257
mem[51788] = 731562
mem[61220] = 6536939
mem[46561] = 882
mask = 010X0110X110X11010101X0X011101001001
mem[34992] = 709195313
mem[629] = 47845214
mem[5317] = 14225
mem[12844] = 723
mem[41998] = 7106433
mem[9010] = 425846
mask = XX11111101111010X0XX1010010X10100XX1
mem[4161] = 30022
mem[47290] = 14778
mem[46760] = 483
mem[42376] = 1286
mem[9346] = 8742
mask = 1101001011X1100010XX11011X0001010X00
mem[5481] = 17524
mem[45359] = 5898
mem[34562] = 1689874
mem[23144] = 250958525
mem[55571] = 7375458
mem[38757] = 370
mask = 01010X101111X1X01011010000010001100X
mem[17410] = 85534302
mem[12631] = 253750
mem[61106] = 613785883
mem[18024] = 34275
mem[48546] = 680
mem[41751] = 140
mask = 1X01X010X01010X0101X10011X0001010100
mem[20073] = 8549591
mem[28358] = 12445
mem[49845] = 8275
mem[23873] = 20426966
mem[18109] = 378612521
mem[55852] = 52955029
mask = X10101110XX0111110XX100X011001101X00
mem[16969] = 474
mem[28358] = 2430
mem[34229] = 51117
mask = 01101111011110XX1X10X0010X01000X1000
mem[20344] = 4618
mem[45257] = 226807155
mem[61651] = 2271
mask = 01XXX1X00110100X10X0111100X100101101
mem[43924] = 1467
mem[34016] = 15
mem[35565] = 97087604
mem[28171] = 595
mem[65534] = 4049200
mask = 100000101111100XX0100000101000001X0X
mem[5481] = 1424699
mem[27180] = 36197
mem[34669] = 131477593
mem[8450] = 37506
mem[4928] = 11393
mask = 01110111011110001X11010010111XX010X0
mem[18923] = 17613
mem[55846] = 20954445
mem[39254] = 51926728
mem[38075] = 130
mem[15418] = 44585145
mask = X00101101111X0101010001X1X01000001X1
mem[35199] = 312510765
mem[62398] = 999656
mem[55533] = 37897408
mem[5648] = 1036966443
mask = 010011110111101XXX1011101000011X1000
mem[19738] = 151629
mem[56289] = 189790
mem[35953] = 15255399
mem[52671] = 396
mem[11845] = 993740
mask = 1101001000111X1X101XX100010001001001
mem[34411] = 295
mem[56671] = 2362723
mask = 1101XX110X10XX1X10101100000001011100
mem[7337] = 8237094
mem[7031] = 118473
mem[39047] = 2581731
mem[51238] = 451786815
mem[18409] = 243
mask = X00101101X10X0101X10011X10X000101000
mem[46760] = 244623
mem[40204] = 3603820
mem[52129] = 3506
mem[16653] = 21230148
mem[41485] = 33094
mem[60015] = 107
mask = X101011001111000101X0X111000X0X00X0X
mem[42738] = 3889911
mem[61025] = 3996
mem[30822] = 319759319
mem[53445] = 6639088
mem[29654] = 4338
mem[21504] = 57158603
mask = X0111111XX11X000101101X1101001100001
mem[5225] = 6975
mem[30877] = 1878
mem[46079] = 1189855
mem[19002] = 3809
mem[18908] = 28205
mem[42747] = 159071873
mem[19144] = 899748
mask = 0101X1110100111X0XX1X1001X1001100000
mem[34377] = 5331828
mem[52109] = 3671
mem[9629] = 10211687
mask = 0101X100XX10X00X111X000111X11X1X0001
mem[62093] = 4555750
mem[22728] = 1902
mem[25369] = 21501187
mem[38470] = 1713
mem[18034] = 9033
mask = X0X101111111X0X1X01011100100000X1110
mem[16231] = 25595
mem[7337] = 64222006
mem[38717] = 17998107
mem[17143] = 56331
mem[3851] = 17
mem[36652] = 810070
mask = 1X00X11111X011101X1111110110X0110010
mem[40873] = 245915859
mem[38090] = 328774
mem[45602] = 361242
mem[40307] = 189025934
mem[55533] = 902335450
mem[48681] = 3398403
mask = 0X011111011010111XX1100001X100001000
mem[27200] = 7180
mem[41393] = 197862524
mem[35565] = 6433
mask = 00111X1X011X100X10X10X0010100X100000
mem[22127] = 2039
mem[49761] = 2974
mem[32236] = 133316
mem[36465] = 233114
mask = 1X0101111X11101X10100100011100001X01
mem[61195] = 12674536
mem[16732] = 11908077
mem[5648] = 46490
mem[15994] = 87271
mem[34229] = 53000
mem[22063] = 966438
mem[59044] = 1966845
mask = 0X01X11101X01X1XXX111000X11001101001
mem[16961] = 3007244
mem[24296] = 229759353
mem[5240] = 248972
mem[33165] = 870504
mask = 110X11110110101X1000X1110100XX0X0111
mem[43104] = 394289736
mem[50640] = 8371
mem[19002] = 22263052
mask = X00X01X0X01X10001X1X1010010110X11001
mem[52725] = 3596
mem[1830] = 852
mem[17859] = 5145419
mem[24109] = 3266
mask = 01001XX1011X1000X0X100X000X11100XX01
mem[2939] = 21652
mem[31549] = 6146
mem[46081] = 89954225
mem[56499] = 566
mem[46561] = 523763
mask = 001X001X111X011110X1X11XX1X01X101100
mem[9682] = 1883
mem[24602] = 937546207
mem[24588] = 2933
mem[11845] = 7846310
mem[33451] = 177708
mem[25861] = 52153
mem[60126] = 6178305
mask = 0100001011111X1110XXX1X0000001101010
mem[32925] = 124064771
mem[47229] = 470344
mem[19578] = 14298
mem[5024] = 281
mem[34016] = 4572362
mask = XX01X1X1011010111011X1111X1011011011
mem[41309] = 6227
mem[23647] = 137
mem[17469] = 322208
mem[55172] = 193476
mem[65367] = 98630465
mem[23822] = 127119
mask = 0101011001111X00X010X0011110X1100101
mem[59963] = 124947188
mem[42597] = 1283704
mem[18482] = 7915
mem[47413] = 471406
mem[43629] = 128188581
mem[63257] = 228254709
mask = 0X11X111011X100X10X10X0111X001101XX1
mem[55436] = 131315318
mem[24551] = 243
mem[4711] = 2883
mem[48157] = 2508
mem[37030] = 21555
mem[34562] = 1589005
mem[11550] = 5293
mask = 0X01011001111X0011100000X10000101X01
mem[14335] = 3968288
mem[44150] = 10819878
mem[29244] = 165332
mask = 01110111011110X01011X10X1X0110101110
mem[59329] = 39829777
mem[34766] = 196214
mem[39624] = 6080817
mem[28054] = 6964
mask = 10010X1011111X001X1X1010101100100X01
mem[37665] = 326246393
mem[43120] = 10019762
mask = 000101XX10111X10101X0100001X1X001X01
mem[51388] = 2236803
mem[18177] = 14993
mem[61543] = 16001841
mem[56229] = 342151958
mask = 0011101XXX111001X001100X111X001X0X01
mem[63605] = 1204
mem[56106] = 24425
mem[8823] = 624
mem[33297] = 24751311
mask = 1101001011XX10101X100X10110011X10X00
mem[1528] = 367894
mem[51697] = 8167
mem[20699] = 149242307
mem[40307] = 3205
mem[10690] = 961
mem[54699] = 10574
mem[57137] = 731
mask = 0X0101001011X1X0101X0000101010101000
mem[53249] = 23260
mem[59520] = 75723443
mem[17105] = 115182
mem[30275] = 6131848
mem[3023] = 400948
mask = 110X101X101010101011X10XX000X100X0X1
mem[11133] = 20188322
mem[21781] = 56747
mem[8420] = 22379
mask = 0100X11101111010101000X010X0001X1X0X
mem[36172] = 53
mem[8450] = 3013
mem[37952] = 3403
mem[16191] = 81318477
mem[13470] = 240413938
mask = 0100X11101111000000X10X00101011X0111
mem[40021] = 57554
mem[64773] = 3803
mask = X001011X1X1X1010101011X0XXXX00101100
mem[41393] = 9033
mem[9481] = 966953
mask = 0100X1X101111000101101X00110010X1000
mem[58752] = 26939
mem[33793] = 109959
mem[22716] = 107238
mask = 1X11X01X0X01100100X101000X00X011X0X0
mem[33454] = 9482389
mem[10379] = 2432
mem[57658] = 265
mem[40140] = 130152807
mask = 01X111100110100X100010010X0110011X0X
mem[27458] = 80396810
mem[37241] = 175138597
mem[12313] = 65106019
mem[27189] = 5555493
mem[15622] = 7712357
mask = X10111110X101X101X110X00X11000101X01
mem[26165] = 623840421
mem[49256] = 9368652
mem[14770] = 544947
mem[31629] = 13938
mask = 000100101111111X011011010X1X101100X1
mem[56248] = 21501
mem[51232] = 61595
mask = 10111001111111XX100X1001110101000X11
mem[29008] = 440
mem[29652] = 270482
mem[38470] = 495
mem[33132] = 1675
mem[17078] = 5234
mem[5001] = 2458
mask = 1X000010111110X010XX0X00X010X00X00X0
mem[30147] = 497416
mem[31711] = 353388
mem[5024] = 51391
mem[42787] = 48397
mem[8066] = 54115
mem[27040] = 754
mask = 0X11111X01X01X0110000101010101100001
mem[55892] = 24149471
mem[42124] = 178547303
mask = X1010101X111X1XX10X01000101X0010XX00
mem[36025] = 95
mem[30529] = 881
mem[64696] = 210812
mem[60217] = 4509
mem[37727] = 14972
mem[3394] = 1633
";
        private const string test = @"mask = XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X
mem[8] = 11
mem[7] = 101
mem[8] = 0";
        private const string test2 = @"mask = 000000000000000000000000000000X1001X
mem[42] = 100
mask = 00000000000000000000000000000000X0XX
mem[26] = 1";
        static void Main(string[] args)
        {
            Console.WriteLine("Hello World!");
            exo2();
        }

        
        private static void exo1()
        {
            var lines = input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);

            long mask1 = 0;
            long mask0 = 0;

            void ParseMask(string mask)
            {
                mask0 = -1;
                mask1 = 0;
                for (int i = 0 ; i < mask.Length;i++) {
                    if (mask[mask.Length - 1 - i] == '1') {
                        mask1 |= 1l << i;
                    }
                    
                    if (mask[mask.Length - 1 - i] == '0') {
                        mask0 &= ~(1l << i);
                    }
                }
                
            }
            var mem = new Dictionary<int, long>();

            void WriteMem(string loc,string val)
            {
                long lVal = Int64.Parse(val);
                lVal |= mask1;
                lVal &= mask0;
                mem[Int32.Parse(loc)] = lVal;

            }

            var maskRegex = new Regex("^mask = (?<mask>[X,0-1]{36})$");
            var memRegex = new Regex(@"^mem\[(?<loc>[0-9]+)\] = (?<val>[0-9]+)$");

            foreach (var line in lines)
            {
                var maskRes = maskRegex.Match(line);
                if (maskRes.Success) {
                    ParseMask(maskRes.Groups["mask"].Value);
                }
                var memRes = memRegex.Match(line);
                if (memRes.Success) {
                    WriteMem(memRes.Groups["loc"].Value,memRes.Groups["val"].Value);
                }

            }

            foreach (var memValue in mem.Values)
            {
                Console.WriteLine(LongToBinaryString(memValue));
            }

            Console.WriteLine(mem.Values.Sum());



        }
        
        private static void exo2()
        {
            var lines = input.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);

            long mask1 = 0;
            List<int> maskX = new List<int>();
            int nbFloating = 0;

            void ParseMask(string mask)
            {
                mask1 = 0;
                maskX.Clear();
                for (int i = 0 ; i < mask.Length;i++) {
                    if (mask[mask.Length - 1 - i] == '1') {
                        mask1 |= 1l << i;
                    }
                    if (mask[mask.Length - 1 - i] == 'X') {
                        maskX.Add(i);
                    }
                }

                nbFloating = 1 << maskX.Count;
                Console.WriteLine(nbFloating);
            }
            var mem = new Dictionary<long, long>();

            void WriteMem(string loc,string val)
            {
                long lVal = Int64.Parse(val);
               
                var adress = Int64.Parse(loc);
                adress |= mask1;

                for (int floatPerm = 0; floatPerm < nbFloating; floatPerm++)
                {
                    var permAdress = adress;
                    var floatAdress = floatPerm;
                    foreach (var pos in maskX) {
                        if ( (floatAdress & 1) != 0) {
                            permAdress |= 1l << pos;
                        }
                        else {
                            permAdress &= ~(1l << pos);
                        }
                        floatAdress >>= 1;
                    }
                    
                    mem[permAdress] = lVal;

                }
                

            }

            var maskRegex = new Regex("^mask = (?<mask>[X,0-1]{36})$");
            var memRegex = new Regex(@"^mem\[(?<loc>[0-9]+)\] = (?<val>[0-9]+)$");

            foreach (var line in lines)
            {
                var maskRes = maskRegex.Match(line);
                if (maskRes.Success) {
                    ParseMask(maskRes.Groups["mask"].Value);
                }
                var memRes = memRegex.Match(line);
                if (memRes.Success) {
                    WriteMem(memRes.Groups["loc"].Value,memRes.Groups["val"].Value);
                }

            }

           /* foreach (var memValue in mem.Values)
            {
                Console.WriteLine(LongToBinaryString(memValue));
            }*/

            Console.WriteLine(mem.Values.Sum());



        }
        public static string LongToBinaryString(long number)
        {
            const long mask = 1;
            var binary = string.Empty;
            while(number > 0)
            {
                // Logical AND the number and prepend it to the result string
                binary = (number & mask) + binary;
                number >>= 1;
            }

            return binary.PadLeft(36, '0');
        }
    }
}